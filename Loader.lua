-- CHEOZ MENU (VERSAO DOS DEUSES - FIX TOTAL)
...
local M,N,P={},{},{}
for Q=1,300 do table.insert(M,G_Draw("Line",{Thickness=1.5,Visible=false}))end;
for Q=1,50 do table.insert(N,G_Draw("Square",{Thickness=1.5,Visible=false}))table.insert(P,G_Draw("Line",{Thickness=1.5,Visible=false}))end;

local R_Skel={{"Head","UpperTorso"},{"UpperTorso","LowerTorso"},{"UpperTorso","LeftUpperArm"},{"LeftUpperArm","LeftLowerArm"},{"UpperTorso","RightUpperArm"},{"RightUpperArm","RightLowerArm"},{"LowerTorso","LeftUpperLeg"},{"LeftUpperLeg","LeftLowerLeg"},{"LowerTorso","RightUpperLeg"},{"RightUpperLeg","RightLowerLeg"}}

local function GetTarget()
    local T,U=nil,j;local V=g(f.ViewportSize.X/2, f.ViewportSize.Y/2)
    for _,Y in pairs(a:GetPlayers())do
        if Y~=e and Y.Character and C_Check(Y)then
...
local function BuildMenu()
    ...
    if ad:FindFirstChild("CheozMobileSystem") then ad.CheozMobileSystem:Destroy() end;
    
    local ae=Instance.new("ScreenGui",ad);ae.Name="CheozMobileSystem";ae.ResetOnSpawn = false
    
    -- O BOTAO "C" QUE VOCÊ PROCUROU:
    ...
    local ai=Instance.new("TextLabel",af);ai.Size=UDim2.new(1,0,0,50);ai.Text="CHEOZ V2 - VERSAO DOS DEUSES";ai.TextColor3=_G.ESP_Color;ai.Font=Enum.Font.LuckiestGuy;ai.TextSize=25;ai.BackgroundTransparency=1

    local function createCol(am)
        local an=Instance.new("Frame",af);an.Size=UDim2.new(0,210,0,280);an.Position=UDim2.new(am,0,0.15,0);an.BackgroundColor3=Color3.fromRGB(22,22,22);Instance.new("UICorner",an)
    ...
    local colCombat = createCol(0.05)
    local colVisuals = createCol(0.53)

    local function createBtn(parent, text, var)
        local b = Instance.new("TextButton", parent);b.Size=UDim2.new(1,0,0,40);b.Text=text;b.BackgroundColor3=Color3.fromRGB(35,35,35);b.TextColor3=Color3.new(1,1,1);b.Font=Enum.Font.GothamBold;b.TextSize=12;Instance.new("UICorner",b)
    ...
    createBtn(colCombat, "ATIVAR AIMBOT", "AimbotEnabled")
    createBtn(colCombat, "TEAM CHECK", "TeamCheck")
    
    createBtn(colVisuals, "ESP BOXES", "ESP_Box")
    createBtn(colVisuals, "ESP SKELETON", "ESP_Skeleton")
    createBtn(colVisuals, "ESP TRACERS", "ESP_Tracers")

    local clrF = Instance.new("Frame", af);clrF.Size=UDim2.new(0.9,0,0.1,0);clrF.Position=UDim2.new(0.05,0,0.85,0);clrF.BackgroundTransparency=1;Instance.new("UIListLayout",clrF).FillDirection="Horizontal";clrF.UIListLayout.Padding=UDim.new(0,8);clrF.UIListLayout.HorizontalAlignment="Center"
    ...
    btnC.MouseButton1Click:Connect(function()
        ...
        if is_open then 
            ...
            tw.Completed:Connect(function() if not is_open then af.Visible=false end end)
        end
    end)
